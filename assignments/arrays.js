...
for (var i = 0; i < inventory.length; i++) { // lenght of inventory is the number of items in the array
    carModels.push(inventory[i]['car_model']) //adds inventory item 0 to the end of car_models
}
carModels.sort(); +
...
carYears.sort(); + 
carYears.reverse(); +
console.log('In decending order:' + ' ' + carYears);
...
console.log('Displays cars older than 2000:' + ' ' + oldCars); 

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = []; 
for (var i = 0; i < inventory.length; i++) {
    var carMake = inventory[i]['car_make'] // finds car make in inventory
    if (carMake === "BMW" || carMake === "Audi"){
        BMWAndAudi.push(inventory[i]);
    }
}
console.log("Here are the BMW and Audis you are looking for:" + JSON.stringify(BMWAndAudi));
